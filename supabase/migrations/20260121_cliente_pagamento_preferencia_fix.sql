-- Ajustes na preferência de pagamento por cliente
-- 1) Função de trigger passa a considerar INSERT e UPDATE e verifica todos os métodos
-- 2) Backfill da tabela com histórico existente

-- Garante criação da tabela caso ainda não exista
...
DECLARE
  ...
  percentual FLOAT;
BEGIN
  -- Só processa quando já está finalizado (em insert) ou mudou para finalizado (em update)
...
-- Recria trigger para INSERT e UPDATE
