...
export default function Dashboard() {
  ...
  // --- FETCH DE DADOS (JUNTOS MAS SEPARADOS) ---
    useEffect(() => {
        // Buscar configuração dos valores dos itens (DESABILITADO - tabela não existe)
        // async function fetchConfig() {
        //   try {
        //     const { data } = await supabase.from('configuracao').select('*').single();
        //     setConfig(data);
        //   } catch (error) {
        //     console.warn('Configuração não encontrada:', error);
        //     setConfig(null);
        //   }
        // }
        // fetchConfig();
        setConfig(null); // Configuração desabilitada temporariamente
  ...
  const picosHorariosFiltrados = useMemo(() => {
    ...
    const perfilVendasMetrics = useMemo(() => {
        ...
        return {
            vendedorasData: vendedorasData.sort((a: any, b: any) => b.valorMes - a.valorMes),
        };
    }, [allVendas]);
                ...
                <Table>
                  ...
                  <TableBody>
                    {tabelaGastos.rows.map((row) => {
                      ...
                      return (
                        <Fragment key={`row-${row.categoria}`}>
